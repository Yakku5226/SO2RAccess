...
> 🚀 **v0.5.5 RELEASED 2026-09-23 19:48** (tag v0.5.5 = dd24b42, 13-file zip, Release build): interactable registry, breadcrumb wall-shortcut fix (+ bridge-safe load cleanup), NavMesh-stuck → breadcrumb retry, F11 floor map / collider / door dump, and the hotfix: spoken directions on the keyboard = apostrophe (`ModAction.NavGuide`, Field context, rebindable, F1 help, README). ✅ TESTED by the user 2026-09-23 evening: the 0.5.5 build works fine (spoken directions on apostrophe, registry rows, prompt speech). Registry/prompt tests 1–6 below count as passed in play.

> 🪐 **SESSION 45 (2026-09-26, afternoon): NEDE WORLD MAP — NO GRID, EVERY WALK REFUSED. PLAN `deep-wibbling-swing.md` APPROVED AND BUILT (0 warnings, DLL in Mods). ⏳ UNTESTED, uncommitted.**
> Log 11:12–11:14 on `mw_1002_01a` (WorldmapID NEDE, player (-153,2.2,-327)): every location `[WMReach] … grid has no Foot data — fail open`, walks "World map grid not found… F9" + "Cannot reach The Centropolis of Nede" (35 m), same on the bunny; "No fishing stands file for nede"; `[GridGen] No embedded/No cached grid` twice every 0.5 s (missing grid was never latched). The bake itself carried Expel numbers: shared bounds X[-1920,1870] Z[-1600,870] and a 12080 (20.8 m) gate fallback height — Nede towns sit at y 1.4–4.5, so F9 on Nede would have fabricated 20 m steps at every gate. User confirmed "land sinks in the distance" = the far-terrain height collapse (chunk loader = the fix, planet-generic); Psynard design included; grid ships embedded after validation.
> BUILT: (1) NEW `WorldmapGridSurvey.cs` — runs first in F9 and on F6: `[GridSurvey]` rect (`worldGridData.RootData.Rect`), culling unit coverage X/Z/Y + pools (helper shared with the F6 dump), every entrance trigger (gate ≤ 20 m / town-wide, CalcHeight), every location symbol (inRect/inUnits), loop members + `ExperLoopObjectCreator`/`WorldMapSilhouetteObjectCreator` child colliders (→ ABORT) + "(Clone)" colliders outside coverage (warning), psynard limits (`fm.psynardSettings` is a struct), derived bounds = rect ∪ coverage + 16 m snapped outward to the 64 m lattice, cap 56 M cells; red flags ABORT with `gridgen_survey_abort`; writes `UserData\SO2RAccess\worldmap_{map}.survey.txt` (kind TAB label TAB x TAB z). (2) `WorldmapGridGenerator`: `GetBakeBounds` — EXPEL keeps the constants (F9 REFUSED while `worldmap_expel.grid` exists: `gridgen_expel_locked`), other maps derived; `[GridGen] Bounds source: …`; progress spoken at 25/50/75 % (`gridgen_progress`); all speech via Loc (`gridgen_*` ×6 langs); `TriggerGroundReference` = median of baked foot-passable cells in the trigger box, else all ground, else CalcHeight, else skip — `[GridGen] entrance … ground …` per gate, "DISAGREES" when grid vs CalcHeight > 5 m. (3) `WorldmapFishingStands.MapName(INVALID)` = null (was "nede"); guards in stands Load, bubble memory, LoadGrid, gate patch, stands baker. (4) `WorldmapPathfinder` grid SLOTS per WorldmapID: missing grid latched (`[WMGrid] {map}: no grid available (latched…)`), loading one planet releases the other (`[WMGrid] released …`), `ClearCache` resets. (5) Per-planet tables `KnownExitsByMap` / `KnownBadStandsByMap` / `KnownWallGroupsByMap` (Nede empty → WARNING "anchors accepted unchecked"); F10 gap points Expel-only; `"Target(Salva)"` → `"Target"`; map names `wm_name_expel`/`wm_name_nede` via Loc. (6) `tools\GridAnalysis.cs`: Expel probes only for Expel files, reads the survey file, 12 m ring-cell search, "regions ≥ 1000 cells". docs/game-api.md §24; memory `nede-worldmap-and-psynard.md`.
> TEST (F12 on; log `<game dir>\MelonLoader\Latest.log`, overwritten each start): (1) Nede world map, open ground → F9. Expect "Generating the nede world map grid…", 25/50/75 %, "Grid saved…" (~2–4 min freeze). "aborted" → send the log (`[GridSurvey] ABORT:` names the reason). (2) F10 at the bake spot; auto-walk to The Centropolis of Nede; F10 at arrival; F7, wait for "route audit complete". (3) Quit; send Latest.log + `UserData\SO2RAccess\worldmap_nede.grid` + `worldmap_nede.survey.txt`. GATES before embedding: survey clean; bake summary plausible (height range near 0–50 m, entranceCellsCleared > 0, no unexplained DISAGREES); offline GridAnalysis (every location has a ring cell; separate regions justified — Phynal/Phynal Pier/L'Aqua/Fun City expected story/psynard-gated); F10 0 ABSENT-AT-BAKE; F7 clean; walks Centropolis + North City on foot and bunny; then Insert stands bake (anchors WARNING until a Nede exit is in `KnownExitsByMap`). Then gzip → `grids\worldmap_nede.grid.gz`, rebuild, version (0.6.0 candidate, user sets).
> ✅ FIRST NEDE BAKE 12:08 (Latest.log 12:07–12:12): survey CLEAN (rect X[-500,500] Z[-500,500], 16978 units / 138 pools, coverage X[-929,681] Z[-703,715] Y[-158,71], 38 triggers = 23 gates + 15 town-wide, 17 symbols, no loop copies, no clone colliders outside coverage, psynard limitY min 3 / expel 130 / nede 71); bounds derived X[-960,704] Z[-768,768] = 3329×3073 (10.2 M cells); probe pass 52 s, 11099 chunk loads; saved `worldmap_nede.grid` 32 MB, height 0.05–71 m; entrance clearing 23 triggers, 1386 un-sealed, 16765 wall cells kept. Gate ground lines: every gate agrees with CalcHeight except the Centropolis (MF_0030 ×3: grid 2.6–4.3 m vs CalcHeight 9–22 m) and Field of Wisdom (1.1 vs 6.4) — the trigger transforms sit on the town model; the grid median is right (the walk arrived at y 2–3.5). F10 ×2 at the Centropolis gate: 0 mismatching cells of 1089 (twice); F11 wall audit PASS 0/66; region 34533 cells, borders 213 ocean / 4542 obstacle. WALKS: Energy Field marker (39 m, one battle interruption, resumed, arrived 1.7 m) and The Centropolis of Nede (ring point, enter prompt) — both ARRIVED. F7: 2 clean (Centropolis, North City), 13 no-route = proven disconnected; list now says "unreachable on foot" for everything but Centropolis + North City. Offline GridAnalysis: foot 554 regions (25 ≥ 1000 cells), bake-spot region = North City gate region (34535 cells); biggest region 1.98 M cells holds no location (the land beyond the barrier). VERDICT: consistent with the game — the Psynard is obtained in North City right after this walk, everything else is flight-only. OPEN: Field of Love symbol + gate (151,45,-89) have NO ground in the grid and live CalcHeight fails there too (its pool may be visual-only, or it needs the psynard landing) — watch in P0. Two MF_0030 dummy triggers at (0,0,0) size 0 (harmless). Still owed: North City walk on foot + bunny, spoken directions, Insert stands bake (14 water places), then embed `grids\worldmap_nede.grid.gz`.
> ✅ USER TESTS 12:17–12:23 ALL PASSED: North City on foot (198 m, arrived, enter prompt); bunny called at North City → bunny walk to the Centropolis (333 waypoints, arrived); spoken directions on the bunny to North City ("Near the entrance… Keep walking North East", arrived); Insert stands bake for Nede: 14/14 places have a stand, 9 proven (unproven 10, 14, 43, 46, 52 — off the foot network), VERDICT PASS, saved `stands\worldmap_nede.json`, anchors WARNING "no logged exits for nede" as designed; walk to Fishing spot 1 (place 8, stand (-216.5,-312)) → creep → bubble → arrived; caught Lord Amberjack + Puny Goby (spoken). Nede exit anchors from the bake (game y = grid y at every gate): Centropolis MF_0030_01A:MAPJUMP_266 exit (-182.5,-325.3), North City MAPJUMP_281 (-74.1,-168.8) → candidates for `KnownExitsByMap[NEDE]` once a real exit is logged.
> 🐛 BUG (user, 12:22): the BAIT SELECTION menu after the catch was silent. Log: `ListSelection: UIFieldFishingBaitListItemPresenter suppressed (dedicated handler owns it)` ×60 — the row type sat on the universal net's covered list but no handler existed (same shape as the session-44 discard bug). BUILT: NEW `FishingBaitHandler.cs` — polls `UIFieldWindow.IsOpened && OpenFieldState == FishingBait`, selector `fishingBaitSelector`; opening (queued): "Choose bait. <place>. Current bait: <name count>. <row>. Catches: <fish chance, …>"; cursor poll → row "Name, xN, size, i of n" + targets read from the visible `UIFieldFishingTargetListItemPresenter` rows 2 frames after the move; tab switch detected via DataCount/first item id. `bait_*` keys ×6. Wired in Main (field, ctor, OnSceneChanged, Update). game-api.md fishing bait subsection. 0 warnings, DLL in Mods 12:4x. ⏳ TEST: fish until the bait runs out (or open the bait menu) → expect heading + place + row + "Catches: …"; up/down speak rows; L1/R1 tab speaks the new list. Log `FishingBait: opened`, `FishingBait: opening idx=…`, `FishingBait: row idx=…`. If "Catches" is missing, the log shows `target rows unreadable` or nothing → read `UIFieldFishingTargetListItemData` via the selector's CreateItemDataList instead.
> ✅ BAIT MENU TESTED 12:33 (log): "Choose bait. Nede (Centropolis Region). Current bait: None. None, -. 1 of 20. Catches: ????(Small) Easy fishing, White Knight Yellowtail Average fishing, …"; every row spoke (Shrimp x14 Small … Two-Star Sardine), target list per row correct; menu closed on decision, silently. User: "generally happy", missing = feedback when a bait is chosen. FIXED (DLL 12:5x, 0 warnings, ⏳ untested): (a) postfix on `UIFieldFishingBaitSelector.OnDecision` remembers the focused row; 0.25 s later, if the menu has closed → "Two-Star Sardine selected." (`bait_selected`), still open → logged "refused"; (b) "????" (fish not caught yet) → "Unknown fish (Small)" (`bait_unknown_fish`); (c) the "None" row's "-" size is no longer spoken. `FishingBaitHandler.ApplyPatches` wired in Main.
> 🔁 SECOND ROUND (log 12:44–12:48, user: "no dismount prompt at all", "still no indication which bait is selected — should work like the discard menu"): (1) the dismount prompt WAS spoken once at 12:45:04 right after the "A Bunny has arrived" dialog (user did not catch it), then every re-show was kept quiet — including after the town's enter prompt at 12:45:15. NEW RULE: while mounted a re-shown prompt is quiet only when it is also the LAST prompt spoken by any presenter (`_lastSpokenRaw`, enter prompt included via `NoteSpokenPrompt`); a mount/dismount (travel mode change) clears it → spoken once per ride, again after another prompt interrupted. (2) Bait decision does NOT close the menu: `decision on row 1 'Shrimp'` → `refused (menu still open)` ×3 — the press marks the bait in place (the 12:33 close was the user backing out). NEW: outcome read from the selector's current-bait header (`baitItemName`) before/after the press → "Shrimp selected." / "Bait removed."; rows now carry "current bait" when `cacheData.fishingBaitItemID/fishingSprinkleBaitItemID == itemID` or the name equals the header; opening says "No bait set" when none. Keys `bait_cleared`, `bait_current_mark`, `bait_current_none` ×6. Log lines `FishingBait: decision on row … (bait before: '…', cache bait=… sprinkle=… isDecision=…)` and `after decision … '…' -> '…'` tell whether the header or the cache ids carry the truth. ⏳ TEST: pick Shrimp → "Shrimp selected."; move away and back onto Shrimp → "Shrimp, x14, Small, current bait"; pick it again → "Bait removed." if the game toggles, else silent + log.
> 🔁 THIRD ROUND (log 12:53–13:01): (1) dismount prompt: the first re-show of the NEW ride (12:55:19) was still quiet — the travel mode was sampled only when a prompt fired, so the dismount at the end of the previous ride was never seen and the new ride counted as the old one. FIX: `TrackTravelMode()` runs every frame in `FieldPromptHandler.Update()` (log `FieldPrompt: travel mode Bunny -> Foot; prompt memory cleared`). The rest worked: after each town enter prompt the dismount prompt spoke once more (12:55:23, 12:55:37), all other re-shows quiet. (2) Bait: rows DID say "current bait" on Shrimp after choosing it (13:01:18) and not on the others — the header `baitItemName` IS the set bait and `cacheData` ids stay -1 (unused). The "selected" confirmation never fired because the OnDecision postfix runs AFTER the game applied the choice, so the header it read as "before" was already the new bait ('Shrimp' -> 'Shrimp'). FIX: `_knownBait` = header value from the previous frame (tracked in Update while no decision is pending) is the real "before". Expected now: "Shrimp selected." on the press; picking Castle Gate Carp afterwards → "Castle Gate Carp selected." (the game replaces, it does not toggle off — 13:01:29). Built, 0 warnings, ⏳ untested.
> 📦 EMBEDDED 15:16: `grids\worldmap_nede.grid.gz` (5.1 MB gzip of the 32 MB 12:09 bake), `stands\worldmap_nede.json` (14 places, 9 proven) and `stands\bubbles_nede.json` (1 point) copied from UserData; csproj wildcards pick them up; Release-style Debug DLL now 34.9 MB (was ~30). README feature line mentions both planets and the Nede land/flight split. NOT committed — commit + version proposal (0.6.0: first Nede support = middle number) awaits the user; the bait "selected" confirmation and the first-prompt-per-ride fix are still ⏳ untested (built 13:0x, no new log since 13:01).
> ✅ FOURTH ROUND TESTED (log 15:35–15:36): dismount prompt spoke 3 s after mounting (`changed`), then quiet; bait menu complete — "Shrimp selected." / "Crest Shrimp selected." on every switch, "current bait" on the set row only, "No bait set" heading. BOTH FEATURES DONE. NEW FINDING: auto-walk to Fishing spot 1 WHILE RIDING → the game's `FieldManager.CheckFishingPoint(bunny)` throws NullReference every frame (85 log lines + IL2CPP stack traces in 1 s) and the mod crept/swept for a bubble that can never show; the user cancelled, dismounted in a bunny-only pocket (foot start region ≠ stand region → straight-line fallback → stuck once → recalc found a foot path → arrived, bubble, catches). FIX (built, 0 warnings, ⏳ untested): fishing arrival while `WorldmapTravel.CurrentMode() != Foot` → `EndFishArrivalMounted()` = stop + "Arrived at {0}. Dismount to fish." (`nav_autowalk_arrived_fish_mounted` ×6); `PlayerFishCheck` returns false without calling the game while mounted; the F12 fishing diag lines say "/p-mounted" instead of calling it. TEST: ride to a fishing spot → one sentence, no error spam in the log; dismount → walk again → bubble as before.
> 🚀 **v0.6.0 (2026-09-26): Nede world map navigation + fishing bait menu.** Commits 4a1128e (Nede: survey, planet-aware bake, grid + stands embedded) and b246a9c (bait menu, riding prompt rule, mounted fishing arrival), then the version bump. Release build, 13-file zip per the recipe (Tolk + nvdaControllerClient64 from the game folder), pushed, GitHub release v0.6.0.
> ✅ MOUNTED FISHING TESTED 15:46: mount → "Press Circle to Dismount" once; ride to Fishing spot 1 → "Arrived at Fishing spot 1. Dismount to fish." (0 CheckFishingPoint errors in the whole log); dismount → walk → creep → player check TRUE → bubble → "Arrived at Fishing spot 1." EVERYTHING FROM SESSION 45 IS TESTED. Uncommitted; awaiting the user's go for the commit and the version (0.6.0 proposed).
> ✅ DISMOUNT PROMPT SPAM FIXED (first version; superseded by the rule above): `FieldPromptHandler` — an unchanged prompt that is merely re-shown while `WorldmapTravel.CurrentMode() != Foot` stays quiet (log `FieldPrompt: re-shown while mounted — kept quiet`, ≤ 1 line / 5 s); a text change still speaks; on foot the ledge-chain re-show rule is untouched. TEST: ride the bunny for a minute → "Press Circle to Dismount" once; approach a town while riding → "Press Cross to Enter" still speaks; dismount and remount → spoken again once.
> PSYNARD (design in the plan, build after the grid gates and once the mount is owned — North City): P0 facts (stick-postfix calls/s while flying, `CanLandingPsynard` transitions, `alightFlag`, L17 census, `MapjumpLayoutData.PsynardPosition` per town) → P1 fly to the town's PsynardPosition, arrival ≤ 6 m AND CanLandingPsynard → "Landing spot. Press {button} to land." (manual landing) → P2 straight bearing + L17 wall probe + L17 wall tones → P3 auto-fly (XZ stick only) if P0 proves the stick is consumed → P4 reachability stays "everything" except live L17-enclosed targets.

> 🗑️ **SESSION 44 (2026-09-26): ITEM DISCARD SCREEN ("storage full, choose items to dump") WAS SILENT. NEW `ItemDiscardHandler.cs`. BUILT, 0 warnings, DLL in Mods 09:35. ✅ USER: "reads fine now" (09:42 play, no F12 log). Uncommitted.**
> ✅ BOTH FIXES CONFIRMED in the 09:52–09:55 log (F12 on): every skill entry logs `CampIC_Action: focus -> #n (stack, hook)` / `(stack)`; L1/R1 right after entering Appraisal, Writing, Mixing, Cooking speak the character at once; Crafting says only its own "Needs Iron"; item discard: `opened (IsOpenedItemDiscardWindow=True)`, 21 rows read, toggle row 7 → "marked=True count=20/20 (was 21)", closed cleanly. User: "All seems to be working correctly now." Uncommitted — commit + 0.5.7 (third number) proposed, user decides. Observed, not changed: in creation skills (Crafting, Cooking) a tab switch fires the creation hook AND the tab name 14 ms apart, so the row text is cut off by the name; Crafting rows say "Iron … Needs Iron" (category = consumed item).
> ➕ THIRD FIX (DLL 10:00, 0 warnings, ✅ CONFIRMED 10:03 log: Claude BP 957, Rena BP 1197, correct on first open and after L1/R1; user: "looks good"): "gibberish number" when opening Improve battle skills (Combat Skill). Log 09:57:45: `CampBattleSkill.enhance … balance='9999'` → spoken "Guardbreak. Level: 7 of 10. BP: 9999 / 95." The mod read the balance from the selector's `skillPointValue` on-screen text, which the game had not filled yet when `UIBattleSkillInformationPresenter.Set` fired — the prefab placeholder "9999" was read. FIX in `ReadSkillPointBalance` (CampMenuHandler.BattleSkill.cs): balance now comes from the game data — `UserParameter.GetCharacterParameter(selector.CurrentPlayerID).CombatSkillPoint` (BP; `SkillPoint` for the specialty screen), on-screen text kept as fallback. TEST: open Combat Skill / Special Arts / Specialties: the number after "BP:" (or SP) must equal the character's real total, on first open and after switching characters. Wording "BP: {balance} / {cost}" unchanged — user may want "costs 95, have 1200" instead.
> ➕ SAME SESSION, second fix (DLL 09:50, 0 warnings): user reported (a) entering Crafting and switching characters with L1/R1 speaks no name until the cursor moves once, (b) a stale "Needs X-clip" (Opera's Customization row) spoken inside Crafting. ROOT CAUSE (code, the 08:57 log was overwritten before it could be re-read): `ResolveFocusedActionSelector` in `CampMenuHandler.ItemCreation.ActionList.cs` only recognised a skill when its action list went from empty to populated or its cursor moved; lists stay populated after leaving a skill, so re-entering Crafting after Customization changed nothing → `_icActiveSelector`/`_icActionListBase` stayed on Customization: tab polling watched the wrong selector, the creation hook read Customization's highlighted row for "Needs …". FIX: `ResolveFocusFromStack()` — the camp window's `selectorStack.Peek()` pointer matched against `_icAllSelectors` is now the primary focus signal (log "CampIC_Action: focus -> #n (stack)"), list heuristic kept as fallback ("(list)"); `SwitchActionFocus()` factored out and also called at the top of the creation-info hook ("(stack, hook)") because that hook fires before the per-frame poll sees the new screen. TEST (F12): Customization → back → Crafting → L1/R1 at once must speak the character; no "Needs" line from another skill; log shows `focus -> #n (stack)` on every skill entry.
> Log 09:19:49: after the overflow toast ("Weird Doll+, Weird Doll", `UIOverflowItemPresenter.SetItem`) the game opened a list of `UICampItemListItemPresenter` rows; every cursor move was swallowed by the universal list net ("suppressed, dedicated handler owns it") but no handler owned it — the screen lives on `UISystemWindow` (state `SystemState.ItemDiscard`, selector `UIItemDiscardSelector : UIListSelectorBase`, rows `UIItemDiscardListItemData : UIItemListItemData` with `isDecisioned`/`isPossession`), not on the camp window, and the camp selector stack stayed on `UICampSpecialSkillResultSelector`. Only the "Dump selected item?" Yes/No dialog spoke (09:20:06).
> BUILT: `ItemDiscardHandler` (EquipWizard pattern): finds `UISystemWindow`, polls `IsOpened && GetCurrentState() == ItemDiscard`; on open waits ≤ 1 s for `DataCount > 0`, then speaks (queued, no interrupt) "Item storage full. Choose items to dump. <game's `operation` text>. <count label>: current of max. <row>"; cursor polled each frame → "Name, xN, new, marked to dump. i of count."; postfix on `UIItemDiscardSelector.OnDecision` and `OnSquare` → next frame "Marked to dump: Name." / "Unmarked: Name." + count, silent when nothing changed (logged). Keys `discard_*` ×6 langs. Main.cs wired (field, ctor, OnSceneChanged, ApplyPatches, Update). `docs/game-api.md` §15 documents the classes.
> TEST (F12 on): craft until the storage-full list appears. Expect: heading + count + first row after the overflow toast finishes; up/down speak rows with "marked to dump" once marked; the decision button says "Marked to dump: X" then "Unmarked: X"; Start still asks "Dump selected item?". Log lines `ItemDiscard: opened (...)`, `ItemDiscard: row [i] ...`, `ItemDiscard.toggle = ...`. If "opened" never appears, the state check is wrong → the log's `GameManager.IsOpenedItemDiscardWindow` value tells which flag to use instead. If OnSquare speaks nonsense, drop that hook.
>
> 🏰 **SESSION 42 (2026-09-23, evening): KROSSE EVENT COPY (MF_0006_01C) — WALK TO THE INN FAILS. ROOT CAUSE FOUND, THREE FIXES BUILT (DLL in Mods 21:21, 0 warnings), ✅ TESTED 21:24 (walk works), COMMITTED aea2631 (local only, NOT pushed, no release).**
> F11 with the new collider list (21:10): the NavMesh sliver at (-58,22.3..23.9) runs through a door mesh collider (`Col_Obstacle_Col1` parent `Mesh_Door_L0`, x -58.9..-57.5, z 23.2..23.8) that is NOT in the game's FieldDoorList → never opens. The `ev_block_cross` boxes are elsewhere (gate→overworld x -5.8..8.3 z -6.9..0.3; north road x -20.5..-10.5 z 51.1..52.9). The user then walked to the inn by hand: low road east to (-41,14), a 1 m ramp up to (-36,17), north along x -34..-29 to the door. Krosse A has the same ramp at the same heights → identical geometry, different NavMesh bake: on the C copy the low road reaches the inn's level only through the closed door. Mod carving ruled out (no NPCs, party skipped, first path before any carver); the game's only NavMeshObstacle user is FieldGimmick16 (boulder), none here.
> BUILT: (1) `NavigationHandler.NavMeshAudit.cs` — F11 `[NAVMESHAUDIT]`: every breadcrumb sampled onto the NavMesh (0.6 m), every walked link `NavMesh.Raycast`; MISS/CUT lines with positions, each cut cross-checked by the wall probe ("NavMesh lies here" vs "link through a wall?"); `debug_navmeshaudit_result` ×6 langs. (2) `traversals\MF_0006_01C.json` bundled (315 crumbs incl. gate→inn). (3) `CalculateAndStorePathCore` step 1b: a complete NavMesh path whose leg crosses a wall face (`NavMeshPathCrossesWall`, WallProbe field profile, 8 m pieces, 0.3 m slack) yields to the recorded route when `_traversal.FindPath` has one — can only swap to a walked route. Log: "NAV: NavMesh path crosses ... taking the recorded breadcrumb route instead".
> ✅ TESTED 21:24 (Latest.log): (a) NAVMESHAUDIT RESULT FAIL, 0 of 315 breadcrumbs off the mesh, 25 of 404 walked links cut, EVERY cut "no wall on the walked link (NavMesh lies here)" — the ramp foot 35->36 (-40,14), the ramp 293->294 / 297->298 (-39.8,14.8)→(-36,17.7), plus ~20 seams along the low road (x -76..-50) = the "disconnected sliver" fragmentation. (b) Walk from the bar door (-77,20): "NavMesh path crosses 'MapCollision (12)'/L22 2.0 m into leg 4" → recorded route, 60 waypoints, ARRIVED in 9.5 s, 0 stuck, no detours. Entering the inn advanced the story: MF_0006_21C → 21A, so the C copy cannot be replayed from this save. (c) Krosse A F11 and (d) a false-fire check on another town: ⏳ still owed. The other player needs this DLL (bundled C recording + the crossing check).
> 🔘 **SESSION 43 (2026-09-24): HOFFMAN RUINS SWITCH CLUTTER — DEAD SWITCHES AND BROKEN BOULDERS LEAVE THE LIST, PRESSED SWITCHES SAY SO. BUILT, 0 warnings, DLL in Mods, ⏳ UNTESTED, uncommitted.**
> Log 09:42–10:24 on `MF_0022_02A` (Hoffman Ruins, flags `FLAG_HOFFMAN_SWITCH_01..25`, `FLAG_HOFFMAN_DESTRUCTION_01..15`): 25 × `FieldGimmick16Switch` + 15 × `FieldGimmick16` (boulders) stayed `listed=True` for the whole session; the Doors list grew 7 → 12 "Boulder N" as broken ones became reachable. The game never deactivates a used switch or a broken boulder. Wrong press order = `FieldGimmick16MultiSwitchFailureTask` spawns an enemy (the Doomaxe fights right after "Press the switch?").
> BUILT: NEW `SwitchPuzzles.cs` (`SwitchPuzzleModel`): one pass over the gimmick list reads every boulder's `MainSwitchID`, `SubSwitchIDList`, `IsMultiSwitch`, `isDestroyed`, `ScenarioFlag` (+ value via `ParameterManager.Instance.UserParameter.GetScenarioFlag`). Rules: boulder broken = `isDestroyed || flag set` → dropped from Doors; switch dead = feeds ≥ 1 boulder and ALL of them broken → dropped from Interactables (a switch feeding nothing stays); switch pressed = its own flag set → label "Switch 3, pressed" (`nav_switch_pressed` ×6 langs, applied after numbering, before ", no path"). `NavItem.NoteKey` + `GimmickHit.Identity/NoteKey` added; switch identity = `switch16:<SwitchID>` (stable numbers by ID). `FieldGimmick11Switch` untouched. Log: `NAV:PUZZLE boulder symbol=… main=… subs=[…] destroyed=… flag=NAME=value broken=…` per boulder, and per switch on the `NAV:GIMMICK` line `puzzle: id=… flag=NAME=value boulders=N broken=M dead=… -> dropped/pressed`.
> ✅ CONFIRMED in the 10:47–11:05 log: 8 of 15 boulders broken → their switches dropped (13 listed → 8 as four more single-switch boulders 8, 9, 11, 12 broke in play); a switch's own flag goes True on a correct press ("Switch 6, pressed") and back to False after the wrong-order ambush. Switch IDs encode the set: boulder N's switches are N×100+1…; boulder 15 = main 1506 + subs 1501–1505 (multi), boulders 1–12 = one switch each (`multi=False`).
> ➕ SETS (user request 11:10, built, 0 warnings, DLL in Mods, ⏳ UNTESTED): every live switch names the boulder it feeds — "Switch 3, boulder 2, pressed" (`nav_switch_boulder` ×6). Boulder identity = `boulder16:<MainSwitchID>`, numbered nearest-first BEFORE the categories are built (`BuildGimmicks` pre-pass) so the note and the Doors row agree; boulders are now always numbered (`alwaysNumbered`). `NavItem.GroupIdentity`, `GimmickHit.GroupIdentity`. The switch's fuse runs on screen from the switch to the boulder's magic circle, so the link is fair. Log now also prints asset names (switch/fuse/circle, rock/circle/other) per object — evidence for whether the press ORDER is visible to sighted players (not spoken).
> ✅ SETS TESTED 11:18–11:41 (log): six switches "boulder 1", singles "boulder 2"/"boulder 3", all three boulders broken, list empty at the end. The winning ORDER was the game's `SubSwitchIDList` order then the main switch (1501→1505, then 1506) after ~8 ambushes. Asset names (`ev_2400600_15_01..06`, fuses `_31..36`, circles `_11..16`, boulder `other=ev_2400600_15_52` only on multi boulders) give no visible-order hint. DECIDED by the user 2026-09-24: the press order stays UNSPOKEN (trial and error, as a sighted player without a visible hint); no main-switch mark, no step toggle. Session-43 work COMMITTED + PUSHED 2c2879b (no release yet; 0.5.7 proposed, user sets the version). Old test list: F12 on, open the nav list before and after pressing each switch of the last puzzle and after the boulder breaks. Expected: solved puzzles' switches and boulders gone; a correctly pressed switch of the open puzzle says "pressed"; after a wrong order (ambush) the "pressed" notes disappear again if the game resets the flags. If the counts are wrong, the `NAV:PUZZLE` lines show which field to use instead.
>
> 🚀 **v0.5.6 RELEASED 2026-09-23 21:40** (tag v0.5.6, Release build, 13-file zip per the recipe): the Krosse event-copy fix (wall-crossing NavMesh route yields to the recorded breadcrumb route, bundled `traversals\MF_0006_01C.json`), F11 NavMesh audit + collider list. Pushed to GitHub.
>
> NEXT SESSION: (1) ask whether the other player confirms the Krosse gate → inn walk on 0.5.6; (2) owed tests (c) Krosse A F11 NAVMESHAUDIT and (d) a walk in another town with no crossing line; (3) other event copies reached in play should be walked once and their `traversals\MF_xxxx_yyC.json` bundled (Arlia 01C already has 84 crumbs in UserData, 24C/26C tiny). The other player's log `2026-09-23_13-13-15.log` sits untracked in the repo root — delete or keep, user's call.
> Another player's log (`2026-09-23_13-13-15.log`, same DLL hash as the release) and the user's own replay (Latest.log 20:55) agree: at story progress 900500 (first arrival, inn = objective, two `ev_block_cross_01_0x` event boxes active) the game loads the EVENT COPY of Krosse. The mod ships breadcrumbs for MF_0006_01A only. On the C copy the NavMesh refuses the direct diagonal road (in A the user's crumbs run straight gate→inn, 38 m) and routes 27 m west through a 1 m sliver at (-58,-1,22.3..23.9) ("snapped to a disconnected sliver" every recalc); the walker stops there (0 m in 2 s), breadcrumb retry has nothing (2 crumbs), 3 detours return into the same sliver, give-up 20:56:26. Probe: `Col_Obstacle_Col1`/L22 0.5 m ahead, `MapCollision (12)` 2.3 m left. Never walked in A (0 crumbs within 2 m of the sliver).
> Gap found: F11's collider/door/trigger list sat after the "too large to draw" early return, so towns logged nothing. FIXED: `NavigationHandler.FloorMapDump.cs` now lists solid colliders within 30 m of the player (`FloorMapListRadius`) plus doors/triggers/stairs on large maps.
> NEXT: reload the Krosse save, F11 at the gate, walk to (-58,23), F11 again → what closes the diagonal and what fills the sliver; ask the other player how they walked to the inn. Fix candidates: (a) walk the C route by hand once and bundle `traversals\MF_0006_01C.json`; (b) carve a NavMesh obstacle over the box the give-up probe finds; (c) prefer the recorded route straight away on event copies.
